...
Edit `drawing_config` in this file, then run:
  python drawing-scalar.py

Outputs:
  1) loglog loss figure: x=M, y=l2_err_rel
...
drawing_config = {
    ...
    "csv_paths": [
        "runs_nopositive/freq-pi/iter300seed42-Sd/iter300seed42sample-saclar-l2-3d-mass-Sd/scalar-d=3-k=1-lstsq-btrans-norotate-nopositive/rate_table.csv",
        "runs_nopositive/freq-pi/iter300seed42-Sd/iter300seed42sample-saclar-l2-3d-mass-Sd/scalar-d=3-k=2-lstsq-btrans-norotate-nopositive/rate_table.csv",
        "runs_nopositive/freq-pi/iter300seed42-Sd/iter300seed42sample-saclar-l2-3d-mass-Sd/scalar-d=3-k=3-lstsq-btrans-norotate-nopositive/rate_table.csv",
        "runs_nopositive/freq-pi/iter300seed42-Sd/iter300seed42sample-saclar-l2-3d-mass-Sd/scalar-d=3-k=4-lstsq-btrans-norotate-nopositive/rate_table.csv",
        "runs_nopositive/freq-pi/iter300seed42-Sd/iter300seed42sample-saclar-l2-3d-mass-Sd/scalar-d=3-k=5-lstsq-btrans-norotate-nopositive/rate_table.csv",
        "runs_nopositive/freq-pi/iter300seed42-Sd/iter300seed42sample-saclar-l2-3d-mass-Sd/scalar-d=3-k=6-lstsq-btrans-norotate-nopositive/rate_table.csv",
        "runs_nopositive/freq-pi/iter300seed42-Sd/iter300seed42sample-saclar-l2-3d-mass-Sd/scalar-d=3-k=7-lstsq-btrans-norotate-nopositive/rate_table.csv"
    ],

    # Output directory
    "out_dir": "runs_nopositive/freq-pi/iter300seed42-Sd/iter300seed42sample-saclar-l2-3d-mass-Sd",

    # Output filenames
    "fig_name": "loglog_loss.png",
    "table_name": "rate_tables.txt",
...
def read_csv_basic(path: str) -> pd.DataFrame:
    ...
    required_cols = {"M", "l2_err_rel", "problem_type", "k"}
    missing = required_cols - set(df.columns)
    if missing:
        raise ValueError(f"[{path}] Missing required columns: {sorted(missing)}")

    df = df.dropna(subset=["M", "l2_err_rel"]).copy()
...
def infer_problem_type(df: pd.DataFrame, path: str) -> str:
    vals = df["problem_type"].dropna().unique()
    if len(vals) == 0:
...
def plot_loglog_loss(dfs: List[Tuple[str, pd.DataFrame]], out_path: str) -> None:
    ...
    for (path, df) in dfs:
        ...
        y = df["l2_err_rel"].to_numpy(dtype=float)
    ...
    ax.set_title("Convergence rate of $L^2$ approximation using $\mathrm{ReLU}^k$, $d=3$", fontsize=14)
...
def make_rate_table_text(df: pd.DataFrame, csv_path: str) -> str:
    ...
    col_logerr = f"{log_base}(l2_err_rel)"
